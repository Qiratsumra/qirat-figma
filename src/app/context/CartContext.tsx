...
export const CartProvider = ({ children }: any) => {
  ...
  const toggleCartItemQuantity = (id: any, value: any) => {
    const foundProduct = cartItem.find((item) => item.id === id);
    const otherProduct = cartItem.filter((item) => item.id !== id);
    if (value === "plus") {
  ...
  const removeItemsFromCart = (product:any)=>{
    const foundProduct = cartItem.find((item) => item.id === product._id);
    const newCardItems = cartItem.filter((item)=>item.id !== product._id);
